...
def filter_numbers(tagged):
    return [item for item in tagged if len(item[0]) > 2
...
def rstopwords(tagged):
    return [(w, t) for (w, t) in tagged if not w
            in nltk.corpus.stopwords.words('english')]


def lemmatize(tagged):
    return [((wnl.lemmatize(item[0]), item[1]) if not ' '
            in item[0] else (item[0], item[1])) for item in tagged]
...
def joincolloc(tagged, colloc_list):
    ...
    for i in range(len(tagged) - 1):
        ...
        if (tagged[i], tagged[i + 1]) in colloc_list:
            sw = 1
            if tagged[i][1].startswith('NN') or tagged[i
                    + 1][1].startswith('NN'):
                tagged1.append((tagged[i][0] + ' ' + tagged[i + 1][0],
                               'NN'))
...
def groupne3(tagged, entity_names):
    ...
    for i in range(len(tagged) - 2):
        if sw == 1:
            sw = 0
            continue
        if tagged[i][0] + ' ' + tagged[i + 1][0] + ' ' + tagged[i
                + 2][0] in entity_names:
            sw = 1
            if tagged[i][1] == 'NNP' or tagged[i + 1][1] == 'NNP' \
                or tagged[i + 2][1] == 'NNP':
...
def joincollocbi(tagged, colloc_list):
    ...
    for i in range(len(tagged) - 1):
        if sw == 1:
            ...
            continue
        if ' ' in tagged[i][0]:
            t1 = ((tagged[i][0])[tagged[i][0].find(' '):].strip(),
                  tagged[i][1])
        else:
            t1 = (tagged[i][0], tagged[i][1])
        if ' ' in tagged[i + 1][0]:
            t2 = ((tagged[i + 1][0])[:tagged[i + 1][0].find(' '
                  )].strip(), tagged[i][1])
        else:
...
def rank_reviews(articles):
    ...
    for linee in articles:
        ...
        tagged_sentences = [tagger.tag(sentence) for sentence in
                            tokenized_sentences]
...
#              wt = float(1.0)/float(len(sentence)) # if weighting by sentence length is desired
    ...
    key_phrases = dict()
    di = sorted(iter(list(calculated_page_rank.items())), key=itemgetter(1),
                reverse=True)
    dic = []
    for (k, g) in itertools.groupby(di, key=itemgetter(1)):
        try:
            w = str(list(map(itemgetter(0), g))[0])
            w = w[:w.find('/')]
            if len(w) > 2 and w not in blacklist:
                if w not in dic:
                    key_phrases[w.replace(' ', '_')] = k
                    dic.append(w)
        except:
            pass
    return key_phrases
